...
public class VKConversation extends VKModel implements Serializable {
	...
	public VKConversation(JSONObject o, JSONObject msg) throws JSONException {
		...
		if (ch != null) {
			...
			JSONObject push_settings = ch.optJSONObject("push_settings");
			if (push_settings != null) {
				...
				no_sound = push_settings.optBoolean("no_souned");
			}
		}
	}

	public static ArrayList<VKConversation> parseArray(JSONArray items, JSONArray profiles, JSONArray groups) throws JSONException {
		ArrayList<VKConversation> c = new ArrayList<>();

		for (int i = 0; i < items.length(); i++) {
			VKConversation con = new VKConversation();

			JSONObject o = items.optJSONObject(i);
			JSONObject conversation = o.optJSONObject("conversation");
			JSONObject last_message = o.optJSONObject("last_message");

			con =  new VKConversation(conversation, last_message);

			if (profiles != null)
				con.profiles = VKUser.parse(profiles);
			if (groups != null)
				con.groups = VKGroup.parseGroups(groups);
