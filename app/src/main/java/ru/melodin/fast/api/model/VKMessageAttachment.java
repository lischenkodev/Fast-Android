 ...
 */
public class VKMessageAttachment extends VKModel implements Serializable {
    /**
     * Types of materials
     */
    ...
    public static VKMessageAttachment parse(JSONObject source, String type, int next_from) throws JSONException {
        ...
        switch (type) {
            case TYPE_AUDIO:
                attachment.audio = new VKAudio(source.optJSONObject(TYPE_AUDIO));
                break;
            case TYPE_VIDEO:
                attachment.video = new VKVideo(source.optJSONObject(TYPE_VIDEO));
                break;
            case TYPE_PHOTO:
                attachment.photo = new VKPhoto(source.optJSONObject(TYPE_PHOTO));
                break;
            case TYPE_DOC:
                attachment.doc = new VKDoc(source.optJSONObject(TYPE_DOC));
                break;
